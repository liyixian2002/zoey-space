...
const objectData = {
    laptop: {
        ...
        content: `
            ...
            <ul>
                <li><strong>AI种草视频自动化生产：</strong>AI视频生产链路的搭建。通过GLM视频理解等12个环节实现低成本爆款视频复刻+自动化生产。已承接闪购松鼠便利、拼好饭等业务视频需求。在cpm、订单转化等前后端指标上跑赢供应商。</li>
                <li><strong>小红书种草图文自动化生产：</strong>参与小红书图文AI生产中台建设。将爆款复刻拆解为图片理解/生图/拼图等原子能力落地为自动化Skill。内容成本由百元级降至1-2元，承接部分业务图文需求，cpm、cpc远低于内部竞品及供应商。</li>
                <li><strong>Agent Team：</strong>主导「有间编辑部」AI营销策划工具从0到1设计，构建6个AI Agent分工协作机制（Agent Team），模拟编辑部多角色评审流程，实现从热点议题到广告投放全流程的Agent化，已完成demo验证。</li>
                <li><strong>舆情监控：</strong>基于各媒体上的业务相关内容做情感倾向分析，辅助业务舆情监控。</li>
            </ul>
...
function setupEventListeners() {
    ...
    // 滚轮支持：仅鼠标滚轮缩放，触控板只旋转
    // Mac 触控板双指滑动和双指捏合都会触发 wheel 事件
    // 鼠标滚轮特征：deltaY 通常为 100 的整数倍，且 deltaX = 0
    // 触控板特征：deltaY/deltaX 值较小且连续变化
    canvas.addEventListener('wheel', (e) => {
        e.preventDefault();
        // 严格判断鼠标滚轮：deltaY 必须是 100 的倍数（标准鼠标滚轮行为）
        const isMouseWheel = Math.abs(e.deltaX) < 1 && Math.abs(e.deltaY) >= 100 && Math.abs(e.deltaY) % 100 < 5;
        if (isMouseWheel) {
            // 鼠标滚轮 = 缩放
